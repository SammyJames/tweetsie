...
impl PinAuth {
	...
	pub fn get_request_token(&self) {
		...
		let key: String = unsafe { 
			String::from_utf8_unchecked(key_insecure.to_vec())
		};
		...
		let to_hash: String = format!("POST&{}&{}", percent_encode(&*REQ_TOKEN_URL), percent_encode(&oauth_str));
		println!("to hash: {}", to_hash);

		let hashed = hmac_sha1(to_hash.clone(), String::new());
		...
		headers.set( 
			Authorization(
				final_header.to_owned()
			)
		);


		let mut res = self.client.post(&*REQ_TOKEN_URL)
								 .headers(headers)
								 .send()
								 .unwrap();
