...
pub fn hmac_sha1(to_hash: String, tok: String) -> String {
    ...
    let secret: String = unsafe { String::from_utf8_unchecked(secret_insecure.to_vec()) };

    let to_combine: (String, String) = (percent_encode(&secret).collect(),
                                        percent_encode(&tok).collect());
